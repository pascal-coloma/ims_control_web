...
export function PatientRegistrationFields({ initialRut, submitting, onSubmit, onCancel }: PatientRegistrationFieldsProps) {
  ...
  function handleSubmit(values: FormValues) {
    ...
    if (values.nombre_completo.trim()) data.nombre_completo = values.nombre_completo.trim()
    if (values.fecha_nacimiento) data.fecha_nacimiento = values.fecha_nacimiento.toISOString().slice(0, 10)
    if (values.direccion.trim()) data.direccion = values.direccion.trim()
    if (values.condicion_paciente.trim()) data.condicion_paciente = values.condicion_paciente.trim()
  ...
  return (
    <form onSubmit={form.onSubmit(handleSubmit)}>
      <Stack gap="xs">
        ...
        <TextInput label="Nombre completo" {...form.getInputProps('nombre_completo')} />
        <DateInput label="Fecha de nacimiento" valueFormat="DD/MM/YYYY" {...form.getInputProps('fecha_nacimiento')} />
        ...
        <TextInput label="Condición del paciente" {...form.getInputProps('condicion_paciente')} />
