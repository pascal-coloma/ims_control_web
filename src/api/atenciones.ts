...
export function getAtencionDocUrl(
  atencionId: number,
...
export async function fetchAtencionDoc(presignedUrl: string): Promise<unknown> {
  ...
  try {
    ...
    throw new Error(
      "No se pudo descargar el documento desde S3 (bloqueado por CORS). Contacta al equipo de infraestructura para habilitar CORS en el bucket.",
    );
  }
  if (!response.ok) {
