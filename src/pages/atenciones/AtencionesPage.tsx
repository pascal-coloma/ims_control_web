...
import { Anchor, Badge, Group, Select, Stack, Table, Text, TextInput, Title } from '@mantine/core'
  ...
  return (
    <Stack gap="md">
      ...
      <Group align="flex-end">
        <Select label="Sello" data={SELLO_OPTIONS} value={sello} onChange={setSello} w={160} />
        <TextInput
      ...
      <Table striped highlightOnHover>
        ...
        <Table.Tbody>
          {filtered.map((a) => (
            <Table.Tr key={a.atencion_id} onClick={() => navigate(`/atenciones/${a.atencion_id}`)} style={{ cursor: 'pointer' }}>
              <Table.Td>#{a.atencion_id}</Table.Td>
              <Table.Td>
                {a.despacho?.paciente ? `${a.despacho.paciente.nombre} (${a.despacho.paciente.rut})` : '—'}
              </Table.Td>
              <Table.Td>{dayjs(a.hora_salida).format('DD/MM HH:mm')}</Table.Td>
              <Table.Td>{a.hora_llegada ? dayjs(a.hora_llegada).format('DD/MM HH:mm') : '—'}</Table.Td>
              <Table.Td>
              ...
              <Table.Td>
                {a.despacho ? (
                  <Anchor component={Link} to={`/despachos/${a.despacho.despacho_id}`} onClick={(event) => event.stopPropagation()}>
      ...
      {selected && <AtencionDetailDrawer atencion={selected} opened onClose={() => navigate('/atenciones')} />}
    </Stack>
  )
}
