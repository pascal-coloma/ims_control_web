...
import { Alert, Anchor, Badge, Button, Divider, Drawer, Group, Stack, Text } from '@mantine/core'
...
import { fetchAtencionDoc, getAtencionDocUrl, getFhirBundle } from '../../../api/atenciones'
import { generatePDF } from '../../../utils/pdf'
import type { AtencionListItem, DocumentoAtencion, EstadoSello } from '../../../types/api'
...
export function AtencionDetailDrawer({ atencion, opened, onClose }: AtencionDetailDrawerProps) {
  ...
  const viewDoc = useMutation({
    ...
    onError: (err) => {
      if (err instanceof ApiError && err.status === 404) {
        ...
        notifications.show({ color: 'red', title: 'Error', message: 'Algo salió mal, intenta nuevamente' })
      }
    },
  })
  ...
  const fhirExport = useMutation({
    ...
    onSuccess: ({ mode, bundle }) => {
      ...
      if (mode === 'download') {
        ...
        notifications.show({ color: 'green', message: 'Bundle FHIR copiado al portapapeles' })
      }
    },
    onError: () => {
      notifications.show({ color: 'red', title: 'Error', message: 'Algo salió mal, intenta nuevamente' })
  ...
  return (
    <Drawer opened={opened} onClose={handleClose} title={`Atención #${atencion.atencion_id}`} position="right" size="md">
      <Stack gap="sm">
        <Text>
        ...
        <Text>
          <Text span fw={500}>Despacho: </Text>
          {atencion.despacho ? (
            <Anchor component={Link} to={`/despachos/${atencion.despacho.despacho_id}`}>
        ...
        <Text>
          <Text span fw={500}>Salida: </Text>
          {dayjs(atencion.hora_salida).format('DD/MM/YYYY HH:mm')}
        </Text>
        <Text>
          <Text span fw={500}>Llegada: </Text>
          {atencion.hora_llegada ? dayjs(atencion.hora_llegada).format('DD/MM/YYYY HH:mm') : '—'}
        </Text>
        <Group gap="xs">
        ...
        <Group>
          ...
          <Button variant="light" onClick={() => fhirExport.mutate('download')} loading={fhirExport.isPending}>
            Exportar FHIR (JSON)
          </Button>
          <Button variant="subtle" onClick={() => fhirExport.mutate('copy')} loading={fhirExport.isPending}>
            Copiar FHIR
          </Button>
        </Group>
