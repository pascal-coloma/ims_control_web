...
import { Alert, Button, Checkbox, Group, Modal, Select, Stack, Switch } from '@mantine/core'
...
export function AssignDispatchModal({ despachoId, opened, onClose }: AssignDispatchModalProps) {
  ...
  const ambulanceOptions = useMemo(() => {
    ...
    const filtered = showAllAmbulances ? data : data.filter((a) => a.estado === 'disponible')
  ...
  const assign = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: queryKeys.despachos.detail(despachoId) })
      handleClose()
    },
    onError: (err) => {
      if (err instanceof ApiError && err.status === 409) {
        setConflictError(err.errorMessage ?? 'Este grupo ya está asignado a este despacho')
      }
    },
  })
  ...
  return (
    <Modal opened={opened} onClose={handleClose} title={`Asignar Despacho #${despachoId}`}>
      <Stack gap="sm">
        <Switch
          label="Mostrar todas las ambulancias"
          checked={showAllAmbulances}
          onChange={(event) => setShowAllAmbulances(event.currentTarget.checked)}
        />
        <Select
          label="Ambulancia"
        ...
        {conflictError && <Alert color="red" variant="light">{conflictError}</Alert>}

        <Group justify="flex-end">
          <Button variant="subtle" onClick={handleClose}>
            Cancelar
          </Button>
          <Button onClick={() => assign.mutate()} loading={assign.isPending} disabled={!ambId || !grupoId}>
            Confirmar
          </Button>
        </Group>
      </Stack>
