...
import { ActionIcon, Button, Card, Group, Modal, NumberInput, Select, Stack, Text, TextInput, Tooltip } from '@mantine/core'
...
function emptyRow(): SupplyRow {
  return { key: nextKey++, nombreInsumo: '', categoriaId: '', unidadMedidaId: '', cantidad: '', stock: '', ambulanciaId: null }
}
...
export function AddSuppliesModal({ opened, onClose }: AddSuppliesModalProps) {
  ...
  const categoriaOptions = useMemo(() => {
    ...
    return Array.from(seen.entries()).map(([id, nombre]) => ({ value: String(id), label: `${nombre} (#${id})` }))
  }, [inventario.data])

  const unidadMedidaHint = useMemo(() => {
    const seen = new Set<string>()
    for (const row of inventario.data ?? []) seen.add(row.presentacion.unidad_medida)
    return Array.from(seen).join(', ')
  }, [inventario.data])

  const ambulanciaOptions = useMemo(
    () => (ambulancias.data ?? []).map((a) => ({ value: String(a.ambulancia_id), label: a.patente })),
    [ambulancias.data],
  )
  ...
  return (
    <Modal opened={opened} onClose={handleClose} title="Agregar insumos (lote)" size="xl">
      <Stack gap="sm">
          ...
          <Card key={row.key} withBorder padding="sm">
            <Group align="flex-end" gap="xs" wrap="wrap">
              <TextInput
                ...
                onChange={(event) => updateRow(row.key, { nombreInsumo: event.currentTarget.value })}
              ...
              <Select
                ...
                onChange={(value) => updateRow(row.key, { categoriaId: value ? Number(value) : '' })}
                searchable
              ...
              <NumberInput
                ...
                onChange={(value) => updateRow(row.key, { categoriaId: typeof value === 'number' ? value : '' })}
                min={1}
                w={100}
                required
              />
              <Tooltip label={`Unidades existentes: ${unidadMedidaHint || '—'}`}>
                <NumberInput
                  label="ID unidad"
                  value={row.unidadMedidaId}
                  onChange={(value) => updateRow(row.key, { unidadMedidaId: typeof value === 'number' ? value : '' })}
                  min={1}
                  w={100}
                  required
                />
              ...
              <NumberInput
                label="Cant/dosis"
                value={row.cantidad}
                onChange={(value) => updateRow(row.key, { cantidad: typeof value === 'number' ? value : '' })}
                min={0}
                allowDecimal
                w={100}
                required
              ...
              <NumberInput
                label="Stock inicial"
                value={row.stock}
                onChange={(value) => updateRow(row.key, { stock: typeof value === 'number' ? value : '' })}
                min={0}
                allowDecimal
                w={110}
                required
              ...
              <Select
                ...
                onChange={(value) => updateRow(row.key, { ambulanciaId: value })}
                searchable
                style={{ flex: 1, minWidth: 120 }}
                required
              />
              <ActionIcon color="red" variant="subtle" onClick={() => removeRow(row.key)} disabled={rows.length === 1}>
                <IconTrash size={16} />
              </ActionIcon>
            </Group>
          </Card>
        ...
        <Button variant="light" leftSection={<IconPlus size={16} />} onClick={() => setRows((current) => [...current, emptyRow()])}>
          Agregar fila
        </Button>
        ...
        <Group justify="flex-end">
          ...
          <Button onClick={() => save.mutate()} loading={save.isPending} disabled={!allComplete}>
            Guardar lote
          </Button>
        </Group>
      </Stack>
