...
export function MoveStockModal({ target, onClose }: MoveStockModalProps) {
  ...
  const move = useMutation({
    ...
    onError: (err) => {
      if (err instanceof ApiError && err.status === 409) {
        setConflictError(err.errorMessage ?? `Stock insuficiente en ${target?.fromPatente}`)
  ...
  return (
    <Modal opened onClose={handleClose} title={`Mover stock — ${target.insumoNombre}`}>
      <Stack gap="sm">
        <Text size="sm">Desde: {target.fromPatente} ({target.fromStock} disponibles)</Text>
        <Select
          label="Hacia"
        ...
        {notFoundError && <Alert color="red" variant="light">Presentación o ambulancia no encontrada</Alert>}

        <Group justify="flex-end">
          <Button variant="subtle" onClick={handleClose}>
            Cancelar
          </Button>
          <Button onClick={() => move.mutate()} loading={move.isPending} disabled={!toAmbulanciaId || cantidad === '' || cantidad <= 0}>
            Mover
          </Button>
        </Group>
      </Stack>
