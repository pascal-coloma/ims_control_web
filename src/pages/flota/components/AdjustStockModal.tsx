...
export function AdjustStockModal({ target, onClose }: AdjustStockModalProps) {
  ...
  return (
    <Modal opened onClose={handleClose} title={`Ajustar stock — ${target.insumoNombre} @${target.patente}`}>
      <Stack gap="sm">
        ...
        <Text size="sm" c="dimmed">
          Resultado previsto: {target.currentStock} + ({typeof delta === 'number' ? delta : 0}) = {result}
        </Text>
        ...
        <Group justify="flex-end">
          ...
          <Button onClick={() => adjust.mutate()} loading={adjust.isPending} disabled={delta === ''}>
            Aplicar
          </Button>
        </Group>
      </Stack>
