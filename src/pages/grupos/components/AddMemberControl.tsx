...
export function AddMemberControl({ grupoId, currentMemberRuts }: AddMemberControlProps) {
  ...
  const add = useMutation({
    mutationFn: () => suscribir({ grupo_id: grupoId, personal_id: Number(personalId) }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.grupos.list() })
      queryClient.invalidateQueries({ queryKey: queryKeys.grupos.detail(grupoId) })
      setPersonalId(null)
      setConflictError(null)
    },
    onError: (err) => {
      if (err instanceof ApiError && err.status === 409) {
        setConflictError(err.errorMessage ?? 'Esta persona ya pertenece a un grupo activo')
      }
    },
  })
  ...
  return (
    <Group align="flex-end" gap="xs">
      ...
      />
      <Button size="sm" onClick={() => add.mutate()} loading={add.isPending} disabled={!personalId}>
        Agregar
      </Button>
      {conflictError && (
