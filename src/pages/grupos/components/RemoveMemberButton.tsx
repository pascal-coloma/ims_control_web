...
export function RemoveMemberButton({
  ...
  const remove = useMutation({
    mutationFn: () =>
      desuscribir({ group_id: groupId, personal_id: personalId as number }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.grupos.list() });
      queryClient.invalidateQueries({
        queryKey: queryKeys.grupos.detail(groupId),
      });
      setOpened(false);
    },
  });
        ...
        <Tooltip
          label={
            personalId === undefined
              ? "No se pudo resolver el ID de personal"
              : "Quitar del grupo"
          }
        >
          <ActionIcon
