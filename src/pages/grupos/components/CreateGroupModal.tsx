...
export function CreateGroupModal({ opened, onClose }: CreateGroupModalProps) {
  ...
  const create = useMutation({
    mutationFn: () => crearGrupo({ nombre_grupo: nombre, personal: personalIds.map(Number) }),
    onSuccess: () => {
  ...
  return (
    <Modal opened={opened} onClose={handleClose} title="Crear Grupo">
      <Stack gap="sm">
        <TextInput label="Nombre del grupo" value={nombre} onChange={(event) => setNombre(event.currentTarget.value)} required />
        <MultiSelect
          label="Miembros"
        ...
        <Group justify="flex-end">
          ...
          <Button onClick={() => create.mutate()} loading={create.isPending} disabled={!nombre.trim() || personalIds.length === 0}>
            Crear
          </Button>
        </Group>
      </Stack>
