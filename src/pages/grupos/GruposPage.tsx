...
import { Badge, Button, Card, Group, Loader, Stack, Table, Title } from '@mantine/core'
...
export function GruposPage() {
  ...
  return (
    <>
        ...
        <Stack gap="md">
          {(grupos.data ?? []).map((grupo) => (
            <Card
              key={grupo.grupo_id}
              withBorder
              ref={id && Number(id) === grupo.grupo_id ? highlightRef : undefined}
              style={
            ...
            >
                ...
                <Table.Tbody>
                  {grupo.miembros.map((m) => (
                    <Table.Tr key={m.rut}>
                      ...
                      <Table.Td>{dayjs(m.dia_ingresado).format('DD/MM/YYYY')}</Table.Td>
                      <Table.Td>
                        <RemoveMemberButton groupId={grupo.grupo_id} personalId={rutToPersonalId.get(m.rut)} memberName={m.nombre} />
                      </Table.Td>
                    </Table.Tr>
                  ))}
                </Table.Tbody>
              ...
              <Group mt="sm">
                <AddMemberControl grupoId={grupo.grupo_id} currentMemberRuts={new Set(grupo.miembros.map((m) => m.rut))} />
              </Group>
            </Card>
          ))}
        </Stack>
      ...
      <CreateGroupModal opened={createOpen} onClose={() => setCreateOpen(false)} />
    </>
  )
}
